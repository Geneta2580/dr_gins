...
void DrGinsInterface::StartDataProcessing() {
    ...
    if (!use_file_data_) {
        // 实时数据模式：订阅ROS话题
        sub_imu_ = private_nh_.subscribe("/imu/data", 100, &DrGinsInterface::ImuCallback, this);
        sub_gnss_ = private_nh_.subscribe("/gnss/fix", 100, &DrGinsInterface::GnssCallback, this);
